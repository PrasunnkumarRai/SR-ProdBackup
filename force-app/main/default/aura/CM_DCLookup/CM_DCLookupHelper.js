({
    ...
    doSearch : function(cmp) {
        ...
        var strFromDCId = cmp.get('v.fromDCId');
        var accId = cmp.get('v.accountId');
		
        if(accId == null || accId=='' || accId == undefined){
        	this.displayToast('Error', 'Advertiser is required for the Campaign.');
            return;
        }
        
        // Create an Apex action
        var action = cmp.get('c.lookup');
		console.log("in helper sobject");
        console.log(sObjectAPIName);
        // Mark the action as abortable, this is to prevent multiple events from the keyup executing
        action.setAbortable();

        // Set the parameters
        action.setParams({ "searchString" : searchString, "sObjectAPIName" : sObjectAPIName, "strFromDCId" : strFromDCId, "accId" : accId });
    ...
    handleSelection : function(cmp, event) {
        ...
        var objectId = this.resolveId(event.currentTarget.id);//To DigitalCampaignId
		var status = '';
        var lstDC = cmp.get("v.matches");
        for (var i = 0 ; i< lstDC.length; i++) {
            if(lstDC[i].Id == objectId) {
                status = lstDC[i].Status__c;
            }
        }
        ...
        updateEvent.setParams({
            "sObjectId" : objectId, "instanceId" : instanceId , "strCampaignStatus" : status
        });
		cmp.set("v.strSelectedDCId", objectId);
        // Fire the event
        updateEvent.fire();
    ...
    clearSelection : function(cmp) {
        ...
        //To Digital Campaign Information hiding
